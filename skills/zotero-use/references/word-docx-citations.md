...
Use this workflow for inserting a minimal Zotero field that Word/Zotero can refresh:
...
If full visual QA is explicitly requested, use the document-rendering workflow available in the user's environment:
...
If `soffice` quits unexpectedly or the renderer reports failure to produce PDF, confirm that LibreOffice, Poppler, and the renderer's Python dependencies are installed and callable from the same terminal.
